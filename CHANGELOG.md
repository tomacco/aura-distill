...
- **`/distill` updates do not fetch the helper** (#78 with #79): the updater's file list (#79) covers the three instruction files and itself. Until it adds `bin/distill-check-store.sh`, only the installers place or refresh the helper; the instructions fall back to the checklist when it is missing or its version marker does not match.
