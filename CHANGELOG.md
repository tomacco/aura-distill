...
### Added
- **Research: files-only memory redesign, measured on real stores** (#75, PR #93): bilingual (EN/ES) page at `docs/research/files-only-redesign.html` testing the redesign's assumptions against two real stores and ten sessions (aggregates only). The always-loaded index is a 51 KB digest, 3.2x its proposed 16 KB budget, grown 57% in seven weeks with no new entries; but moving dated history brings only 3 of 56 oversize files under the cap (splitting does the work), the lifecycle would archive 2 of 23 project files, and 18 of 20 existing archive files are nested, which the design's migration would refuse. Introduces the shared `docs/assets/ne-editorial.css` stylesheet. No behaviour changes.
