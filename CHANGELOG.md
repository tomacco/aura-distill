...
- **`/distill` updates deliver only part of this change** (#78 with #79, fix in #113): the updater's file list covers `distill.md`, `distill-process.md`, `distill-monitor.md` and itself. It does not fetch `bin/distill-check-store.sh` (the distiller falls back to its checklist when the helper is missing or its version marker does not match) nor the Claude rules file `rules/distill.md` (its new compact retrieval rules arrive only by re-running the installer; the monitor carries the full protocol, so retrieval degrades rather than breaks). The Codex block and the Antigravity plugin are also installer-only.
