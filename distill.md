...
and run the Mode they choose. The ordinary run's own Step 3 (which happens before the Mode runs) records its ledger line with `"mode":"skipped"`: the conversation was queued, not distilled (the inbox item is encoded by the next distillation after the migration).
...
- Signals processed: N
- Inbox: K items consumed (0 if none; one line each when K > 0); if a pending migration blocked encoding, say so and give the inbox path the harvest was queued to; for every learning queued because its file is archived on a store not yet migrated, give its inbox path and "run migrate-store, then restore X"
