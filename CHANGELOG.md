...
### Added
- **Capabilities-matrix skill** (`skills/capabilities-matrix/`): local, private model calibration implementing capabilities/METHODOLOGY.md — 8-step supervised flow (discover → scope → dry-run estimate gate → generate private probes from per-demand recipes → run → blind judge → discriminativeness gate → write contracts). Probe-generation recipes for D3–D8 ship as shape-specs (never probe instances — contamination-resistant by design). Bundled `battery-runner.py` mechanically enforces the hard cost cap (aborts and reports results as PARTIAL), deterministic blinding with a sealed arm mapping (unseal refuses until scores exist), fail-fast on rate limits, idempotent cells, and per-demand spread/dispersion reporting. Custom endpoints supported (OpenAI-compatible HTTP); endpoint output is treated as data, never instructions. Installer sections both platforms with full user control (`--capabilities-skill/--no-capabilities-skill/--remove-capabilities-skill`; `$env:DISTILL_CAPABILITIES_SKILL`), no-clobber, download validation. Results live in `{DISTILL_DIR}/data/capabilities-local.json` and never leave the machine.
