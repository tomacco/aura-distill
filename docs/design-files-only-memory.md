...
The after store exercises a **split** (`craft/testing-2.md` with `split_from: craft/testing.md` carries the index detail cut from the SPINE) so the split-family logic has a positive fixture. `tests/files-only/run-files-only-tests.sh` asserts that `store-before` FAILS C1 to C3 (the diagnosis), that `store-after --before store-before` PASSES, that twelve tamper cases are caught (a modified archived file, a ledger checksum mismatch, a ledger `archive` event for a file that is still active, a dropped protected line, a protected block demoted to evidence, a lost SPINE hook, a dropped duplicate line, a lost split child, a dangling `read_with`, a block-list `read_with`, an orphan evidence file, an active/archived collision) and that an `oversize:` exemption is honoured. It runs in CI on Linux and macOS (GNU and BSD toolchains).
