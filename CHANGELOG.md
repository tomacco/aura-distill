...
### Added
- **Research: files-only memory redesign, measured on real data** (#75, PR #93): bilingual (EN/ES) page at `docs/research/files-only-redesign.html` testing the redesign's assumptions against one real store in two snapshots, a second store's index and ten sessions (aggregates only). The always-loaded index is a 51 KB digest, 3.2x its proposed 16 KB budget, grown 65% since Aug 1 (to Sep 22) with no new entries; moving dated history brings only 3 of 56 oversize files under the cap (splitting does the work), the lifecycle as measured would archive 2 of 23 project files, and 18 of 20 existing archive files sit under `archive/<tier>/`, which migration would have refused. Rounds six and seven of the design review adopted all four findings. Introduces the shared `docs/assets/ne-editorial.css` stylesheet. No behaviour changes.
