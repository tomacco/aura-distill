...
**Legacy archives.** Files produced by the old rewrite-style compaction (F4, F5) sit flat under `archive/` with no ledger line. Legacy is **structural**, not a label: a file directly under `archive/` is legacy, a file under `archive/<tier>/` never is, and the catalog's `legacy` label must agree with the path. Legacy files are kept as they are, never deleted (Tier 3's cardinal rule), and never checked for identity against a source that no longer exists; they are checked for identity against their own pre-migration copy.
...
2. **Backup.** On `--apply`, copy `SPINE.md`, every tier-2 file and every pre-existing evidence twin the plan will append to into `data/migration/<ts>/backup/` and write `PENDING`. Only then edit.
...
4. **C4** (with `--before`) Every non-blank line of every original tier-2 file **and every original evidence twin** appears **at least as many times** in the after store (principle file, evidence twin, split children, archived copy); every protected bullet (dated forms included) **and its indented block** still exists in an active or archived file; every archived file that is not legacy is byte-identical (sha256) to its original, or, when it was legitimately edited after migration and before a later move (an Index detail appended, say), to its ledger checksum, its original lines then being covered by conservation; every file that was under `archive/` before still exists there and every legacy file is byte-identical to its own pre-migration copy; a pinned file and any file carrying `[NON-NEGOTIABLE…]` were not moved; every original SPINE hook survives as a substring in `SPINE.md`, a tier file, an archived knowledge file or the catalog (never counting `data/`, evidence, or the ledger itself; for archived files the catalog row carries the hook by construction, so that case is guaranteed by the rebuild rather than tested independently).

The after store exercises a **split** (`craft/testing-2.md` with `split_from: craft/testing.md` carries the index detail cut from the SPINE) so the split-family logic has a positive fixture, and the runner exercises a **restore** (atlas moved back with a `restore` ledger line, its saved entry trimmed to the cap, the cut wording appended under `## Index detail`, catalog row moved to active) for both the plain and the `restore (modified)` event, and asserts that a file carrying `[NON-NEGOTIABLE]` cannot be archived. `tests/files-only/run-files-only-tests.sh` asserts that `store-before` FAILS C1 to C3 (the diagnosis), that `store-after --before store-before` PASSES, that twenty-six tamper cases are caught (the twenty below plus: a deleted legacy archive file, a nested archive file labelled `legacy` with the ledger removed, a line dropped from a pre-existing evidence twin, a dated `[NON-NEGOTIABLE date]` block demoted to evidence, a heading-form `[DIRECTIVE date]` section demoted to evidence, an evidence twin whose `evidence_for` disagrees with its path) and that archiving one target of a merged multi-pointer line by pointer removal passes (a modified archived file, a ledger checksum mismatch, a ledger `archive` event for a file that is still active, a stale catalog `rebuilt:` stamp, a dropped protected line, a protected block demoted to evidence, a lost SPINE hook, an Index detail holding only the tail of a hook, a dropped duplicate line, a lost split child, a dangling `split_from`, a dangling `read_with`, a `read_with` into `local/`, a block-list `read_with`, an orphan evidence file, an active/archived collision, a catalog `pinned` flag or `validated` date that disagrees with the file, an after-shaped SPINE with one entry over the cap, a `[NON-NEGOTIABLE]` file archived) and that an `oversize:` exemption is honoured. It runs in CI on Linux and macOS (GNU and BSD toolchains).
