...
**{DISTILL_DIR}** = the durable knowledge directory for `aura-distill`. Resolve in order:
1. Environment variable `$env:AURA_DISTILL_DIR` / `$AURA_DISTILL_DIR`
2. Workspace directory: `.agents/distill` (if present)
3. User home: `~/.aura-distill` (Windows: `%USERPROFILE%\.aura-distill`, POSIX: `~/.aura-distill`)
...
2. **Recency view:** Run `{DISTILL_DIR}/bin/distill-recent-agy.ps1` (or `distill-recent.ps1`) to view human-bucketed session history.
