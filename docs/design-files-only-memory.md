...
| F13 | The harness cannot observe reads. The shipped tier-2 format has no file-level `last_validated`; per-principle `last_validated` lines are bumped on confirmation, and the old staleness review bumps `last_updated` when the user answers "still relevant". Validate-on-read also bumps `last_validated`. None of these is a read count. | `distill-process.md` Step 3a, Step 5 "Staleness review", "Tier 2 file format"; `knowledge-architecture.md` "Memory Recall Protocol" |
...
The ledger line carries everything a restore or a second machine needs: the checksum, the reason, and the **full removed SPINE line**, so restore re-adds exactly the entry that was removed and nothing has to be invented. The catalog (D4) derives its archived rows from the ledger; a rebuilt catalog therefore never loses provenance, and a machine that receives the archive through sync receives the ledger with it.
...
Every field is derived: `scope:` and the newest date stamp from the file's frontmatter, `pinned` from `lifecycle:`, archived rows from the ledger (including the original hook, which is what a miss search needs to match), evidence counts by counting `- ` lines. The rebuild is therefore mechanical and a developer check asserts the catalog equals the tree, including counts.
...
4. The catalog is **stale** when any catalog line points at a missing file, or when any file under the tier directories, `archive/`, `evidence/` or `SPINE.md` is newer than `CATALOG.md` (a `find -newer` in the harness; no date field is needed). A stale catalog is named as such in the answer.
...
| Exempt | Frontmatter `lifecycle: pinned` (also echoed as the word "pinned" in the SPINE hook so old clients' staleness review sees it); any file outside `projects/` (craft, ops, profile, feedback keep ask-first, because a stale principle is not a finished project) |
...
| Restore | `/distill restore <path>` moves the file back, checks the ledger checksum (restore anyway on mismatch, report it), appends a `restore` ledger line, re-adds the saved SPINE entry, rebuilds the catalog |
...
`[NON-NEGOTIABLE]` and `[DIRECTIVE]` markers do not block a move, because a move preserves wording verbatim (F7 forbids dropping or compressing, not relocating), and a directive-bearing file is retrievable through D5 the next time its project comes up. **Directive applicability** (who issued it, whether it has expired) is preserved as the existing `origin: directive (who, when)` wording and is otherwise deferred to Track B (#82), see section 8.
...
| `archive/**` including `archive/LEDGER.md` | shared | knowledge and its provenance |
...
4. **Interrupted.** A later run that finds `PENDING` offers two mechanical choices. **Finish**: resume at the first file not marked done (the idempotent append makes a half-done file safe). **Revert**: reverse every move listed in the plan, delete every path the plan created, then copy `backup/` over the store. Both are copies and deletions from the plan, not judgement.
...
2. **C2** Every SPINE pointer resolves; every active tier-2 file has a SPINE pointer; a catalog exists and lists every active, archived and evidence file with no line pointing at a missing file; evidence counts in the catalog equal the `- ` lines in each twin; every `evidence_for` names an active or archived file; no path exists both active and archived; every non-legacy archived file has a ledger line whose sha256 matches the file; archived catalog rows carry `from` and `hook`.
...
4. **C4** (with `--before`) Every non-blank line of every original tier-2 file appears **at least as many times** in the after store (principle file, evidence twin, split children, archived copy); every protected bullet **and its indented block** still exists in an active or archived file; every archived file that is not legacy is byte-identical (sha256) to its original, and every legacy file is byte-identical to its own pre-migration copy; a pinned file was not moved; every original SPINE hook survives as a substring in `SPINE.md`, a tier file, an archived file or the catalog (never counting `data/` or evidence).

`tests/files-only/run-files-only-tests.sh` asserts that `store-before` FAILS C1 to C3 (the diagnosis), that `store-after --before store-before` PASSES, and that ten tamper cases are caught: a modified archived file, a dropped protected line, a protected line demoted to evidence, a lost SPINE hook, a dropped duplicate line, a dangling `read_with`, a block-list `read_with`, an orphan evidence file, an active/archived collision, and a ledger checksum mismatch. It runs in CI on Linux and macOS (GNU and BSD toolchains).
...
| Archive provenance survives rebuilds and sync | Written to the synced ledger at move time; catalog derives from it | File convention; verified by C2 |
