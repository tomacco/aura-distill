...
A protected bullet and its **indented continuation lines** (`confidence:`, `origin:`, `Why:`) stay together in the principle file; the checker protects the block, not just the first line. A protected marker is any bracket that **starts** with `NON-NEGOTIABLE` or `DIRECTIVE`, so the dated forms real stores carry (`[NON-NEGOTIABLE 2026-05-02]`, `[DIRECTIVE 2026-08-02]`, the shipped `[UPDATED date]` convention) count; the pattern is `\[(NON-NEGOTIABLE|DIRECTIVE)[^]]*\]` everywhere it is used (checker, lifecycle exemption, runtime self-check).
