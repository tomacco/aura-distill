...
### Fixed
- **Updater follow-ups** (#79): store metadata strips only a *leading* byte-order mark and trailing CRs, so non-ASCII paths (`û`, `»`, …) are no longer corrupted; a CRLF `preferences.md` no longer silently turns auto-update off; `.command-path` now lists every Claude profile sharing the store and each one's `/distill` is updated; the dispatcher's bootstrap step says which substitution is allowed; a profile whose `distill.md` was removed stays uninstalled; `/distill` updates now also refresh `rules/distill.md` with the installers' preference-preserving merge (your Always-On preferences are kept byte for byte; all three now keep any section content that is not the untouched template, not only sections with a bold rule line, which fixes a loss of bullet-only or prose preferences present since 1.1.8); the updater also installs or refreshes the optional store checker (`bin/distill-check-store.sh`, #78) when a release carries it. CI runs the Windows path end to end (`install.ps1`, then the updater under Git Bash: `tests/updater-compat/windows-smoke.sh`).
