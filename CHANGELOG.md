...
- **Optional store self-check helper** `bin/distill-check-store.sh` (#78; the checker formerly at `tests/files-only/check-store.sh`): the installers copy it to `~/.aura-distill/bin/`, and the distiller runs it when bash is available (`--print-catalog` rebuilds the catalog, `--hashes` prints the raw and `recall_count`-normalised checksums). Without bash the distiller checks the same invariants as a checklist and says so. New checks: ledger lines in date order, retrieval-marker lines never only in evidence, the catalog line anchored to an entry, `recall_count` drift tolerated through `sha256-norm:`. New in review round one: `local/SPINE.md` may point only inside `local/`. The suite grows from 103 to 123 checks.
