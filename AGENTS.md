...
- Run the files-only store invariants (thin SPINE, catalog completeness, lossless migration; #75): `bash tests/files-only/run-files-only-tests.sh` — design in `docs/design-files-only-memory.md` (seconds on Linux/macOS, ~3 minutes on Windows Git Bash because of process-spawn cost)
