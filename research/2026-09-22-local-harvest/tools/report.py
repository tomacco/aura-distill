#!/usr/bin/env python3
"""Assemble the run + judge artifacts into the results table + a machine-readable results.json."""
import glob, json, os
...
rows = []
for m in sorted(glob.glob(f"{RUNS}/*.meta.json")):
    label = os.path.basename(m).replace(".meta.json", "")
    d = json.load(open(m))
    sc = f"{JUDGE}/{label}.score.json"
...
for r in rows:
    r["pct_of_opus"] = round(100 * r["recall"] / ceiling) if (r["recall"] is not None and ceiling) else None

json.dump({"ceiling_recall": ceiling, "rows": rows}, open(os.path.join(ROOT, "results.json"), "w"), indent=1)
...
for r in rows:
    print("| {label} | {where} | {wall_s} | {input_tokens} | {output_tokens} | {gen_tps} | {peak_memory_gb} | {cost} | {recall} | {pct} | {grounding} | {fabrication} |".format(
        ...
        pct=f"{r['pct_of_opus']}%" if r["pct_of_opus"] is not None else "—", **r))
